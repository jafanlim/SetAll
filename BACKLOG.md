...
- **edge-fn-secret-enforcement** (security, P1) — 7 edge fns deploy `verify_jwt=false` with NO `x-edge-secret` check → publicly invokable (notification spoofing / Groq+email quota abuse). C-1 made the DB triggers SEND `x-edge-secret`; the functions didn't VERIFY it.
  - ✅ **3 of 7 DONE** via PR #23 (2026-06-25, squash `53d10a9`): `bug-triage`, `monthly-digest`, `send-group-notification` — gated with an `x-edge-secret`/`EDGE_SHARED_SECRET` fail-closed check (after OPTIONS, before work). Controller-verified: exactly 3 `index.ts`, deferred fns untouched, `deno check` + analyze + 343 tests green. **⚠ OPS REQUIRED before redeploy:** `supabase secrets set EDGE_SHARED_SECRET=<vault edge_shared_secret value>` then `supabase functions deploy bug-triage monthly-digest send-group-notification` — else the triggers 401 (no bug-triage email / group notifications / monthly digest).
  - ⬜ **Deferred 4 (each needs more than an x-edge-secret-only gate — controller triage 2026-06-25):**
    - `weekly-analysis` — DUAL caller: C-1 cron (sends `x-edge-secret`) **+** app on-demand (`insights_screen.dart:1164`, user JWT, no secret). Needs an **x-edge-secret OR authenticated-user** guard (verify the Supabase JWT in-function), not secret-only.
