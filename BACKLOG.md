...
- **`chore/shared-wiring`** (origin `bc7e056`) — KEEP until C-9 done. Its data-layer is on develop (C-4/PR #15),
  ...
  **✅ FULLY HARVESTED (2026-06-25).** All four slices (ingest C-5, recurring C-7, budget C-8, alerts C-9) + the data layer (C-4) are on develop. Nothing of value remains in `bc7e056` EXCEPT the settings entry-tiles (recurring/budgets/alertPrefs, ~lines 643-659) still to be lifted in C-10. **READY TO PURGE** — defer the actual delete to Phase-0 close-out, and `git tag archive/shared-wiring bc7e056` (push tag) BEFORE deleting the branch so the SHA stays referenceable (this ledger cites its line numbers throughout).
