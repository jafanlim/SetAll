...
- ✅ **DONE (already fixed — controller recon 2026-06-25)** **net-balance-offset** _(TASK 3)_ — A↔B mutual debts net to zero. The fix is live: `balance_service.dart:50` nets `net = rawOwed − rawOwe` (then zeroes the smaller side) at BOTH global and per-group level — introduced in `2ccbf76` ("split math fixes"). Covered by `engine_torture_test` TEST 2 (summary-level: owed $50 / owe $20 → `youAreOwed 30.00`, `youOwe 0.00`) + TEST 3 (anchor-equal → 0.00), plus the settlement engine's per-pair netting (`repository_crud_test:866`, `engine_regression_guard 2a`). (Minor leftover: `friends_screen.dart:331`'s `owed>0 && owe>0` branch is now dead post-netting — harmless cosmetic cleanup, not the bug.) **UPDATE (PR #40, TASK 3 dispatch 2026-06-27):** the prior recon said "no dispatch needed", but there was **no dedicated Math-Guard suite** (the Makefile `test:` target pointed at a non-existent `balance_service_test.dart`). Dispatched DeepSeek to write it → added `test/core/services/balance_service_test.dart` + `settlement_engine_test.dart` (26 hermetic tests incl. anti-vacuous; `make math-guard`). The suite **caught a real pre-existing sub-cent defect**: `SettlementEngine` emitted a spurious `0.00` settlement txn when a member's net rounded sub-cent (the `payment > 0` gate used the *unrounded* amount) → fixed (round-first + skip/advance, guarantees termination). Netting itself confirmed correct at both layers. 409/409.
...
- 🔨 **statement-multi-import** _(TASK 6)_ — reconcile with wallet-csv-import; split + dedupe + dates. **6a DONE (PR #38, `feat/statement-import-dedupe-c`):** importer (parse/review/commit/date-preserve) already landed via PR #16; 6a adds dedupe — shared `importDedupSig` (`lib/core/utils/import_dedup.dart`, extracted from splitwise `_dedupSig`), `IngestService.flagDuplicates` pre-rejects+badges rows matching existing wallet entries, `IngestRow.isDuplicate` + "Duplicate" chip, `ingest.duplicate` ×6 locales. Controller fix: `commitApproved` `!isDuplicate` guard blocked override of a false-positive → made status-driven + override test. Decimal/cents, 383/383. **6b deferred (infra/deploy-gated):** receipt-scanner statement-detection + array response in `netlify/functions/receipt-ingest.js` so a photographed statement routes to the multi-row flow instead of collapsing to one expense.
- 🔨 **shared-expense-wallet-share** _(TASK 5)_ — opt-in mirror of my share into wallet. **5a–5c-ii DONE (5c-iii optional).** 5a (PR #32): `source_expense_id` on `expenses` (migration `20260626000001` applied to prod — uuid self-FK `ON DELETE SET NULL` + partial index; SQLite v37; `upsertWalletEntry` dedupe). 5b (PR #33, `feat/wallet-share-mirror-c`): `myShareFromResults` (Decimal, reads `amountOwed`) + ask/always/never SharedPref + post-save confirm sheet that mirrors via `upsertWalletEntry(…, sourceExpenseId)`. 5c-i (PR #34, `feat/wallet-share-propagate-c`): edit/delete propagation at repo layer (`_removeMirrorForSource` on delete, `_propagateEditToMirror` on edit). Controller fixed 2 workhorse misses (swallow-all `_findMirrorId`; edit reset mirror `created_at`→now). 5c-ii (PR #36, `feat/wallet-share-badge-c`): `deleteGroup`/`forceDeleteGroup` cascade removes linked mirrors (owner · non-owner leave · force-purge, web+native) — **orphan-mirror gap closed**; `sourceGroupName` + read-only `GroupBadge` "from <group>" chip (list + detail). Controller fixed 2 more misses: web FK-ordering (`_removeMirrorForSource` ran after source delete, but `source_expense_id ON DELETE SET NULL` nulled the link first → orphan survived → moved before cascade on web paths; native unaffected/non-hermetic so SQLite tests passed) + `GroupBadge` `ProviderScope.containerOf` `listen:true` in `initState` threw-then-swallowed → always "from a group" in debug → `listen:false`. Decimal-only, 369/369. **Next:** 5c-iii (mirror activity-log events) — OPTIONAL/deferred (`_buildOmniActivity` skips wallet entries; needs dedup design).
- ✅ **DONE — stale-confirmed (PR #42)** **web-insights-datasource** _(TASK 4)_ — web reads real amounts (no SQLite). The Supabase datasource was wired Feb/Mar 2026 (`getRecentExpenses`/`getPersonalExpenses` `_isWeb` paths → `analyticsDataProvider` → insights, via source-agnostic `ExpenseModel.fromJson`). No production change; added the missing 26-test analytics guard suite (`test/features/analytics/`). Controller fix: 3 dedup tests hardcoded `2026-06-20` → would age out of the rolling 30-day window → switched to relative `_daysAgo(5)`. Flagged for a separate task: `analyticsDataProvider` aggregates in `double` (Decimal money-rule violation).
...
- **group-info per-member debt amounts** _(NEW P2, user req 2026-06-26)_ — ✅ **DONE** (PR #30, `d029731`). Members card shows per-member "owes you / you owe / settled up" from `simplifiedDebtsProvider`; Decimal preserved, current-user excluded. Controller-verified.
