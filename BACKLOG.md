...
- "settled up" stale after account switch — 🔄 **IN PROGRESS (P1-D)**: root-caused to `lib/app.dart` `_invalidateAllProviders()` (L263) omitting the per-group/wallet/master balance providers (`groupBalanceSummaryProvider` family drives the per-group "Settled up" badge); fix = add the missing financial providers + make the post-sync re-invalidation comprehensive on switch/first-login only.
...
- **edge-fn-secret-enforcement** (security, P1) — 7 edge fns deploy `verify_jwt=false` with NO `x-edge-secret` check → publicly invokable (notification spoofing / Groq+email quota abuse). C-1 made the DB triggers SEND `x-edge-secret`; the functions didn't VERIFY it.
  ...
  - ⛔ **EDGE_SHARED_SECRET mismatch (gated secret op — user runs).** Post-apply digest test returns **401** (our x-edge-secret gate, not 403). SHA-256 compare (no secret exposed): edge-fn env `EDGE_SHARED_SECRET` (`64f6…`) ≠ Vault `edge_shared_secret` (`803e…`). Fix: `supabase secrets set EDGE_SHARED_SECRET=<exact Vault edge_shared_secret value> --project-ref vrsmsgyxeyzyrdonsnrk` (no redeploy). Then re-test → 200 + email. This supersedes PR #23/#24's "OPS REQUIRED" note above (the set step was either skipped or set to a different value). **TASK 2 (push-and-digest) blocked until this flips.**
