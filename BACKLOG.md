...
- 🔨 **shared-expense-wallet-share** _(TASK 5)_ — opt-in mirror of my share into wallet. **5a + 5b DONE.** 5a (PR #32): `source_expense_id` on `expenses` (migration `20260626000001` applied to prod — uuid self-FK `ON DELETE SET NULL` + partial index; SQLite v37; `upsertWalletEntry` dedupe). 5b (PR #33, `feat/wallet-share-mirror-c`): `myShareFromResults` (Decimal, reads `amountOwed`) + ask/always/never SharedPref + post-save confirm sheet that mirrors via `upsertWalletEntry(…, sourceExpenseId)`. 5c-i (PR #34, `feat/wallet-share-propagate-c`): edit/delete propagation at repo layer (`_removeMirrorForSource` on delete, `_propagateEditToMirror` on edit). Controller fixed 2 workhorse misses (swallow-all `_findMirrorId`; edit reset mirror `created_at`→now). Decimal-only, 362/362. **Next:** 5c-ii activity events + wallet badge. **Gap:** `deleteGroup` cascade doesn't remove linked mirrors (orphan mirrors when a whole group is deleted) — fold into 5c-ii.
