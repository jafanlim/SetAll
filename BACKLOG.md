...
- 🔨 **shared-expense-wallet-share** _(TASK 5)_ — opt-in mirror of my share into wallet. **5a data-layer DONE** (PR #32, `feat/wallet-share-link-c`): nullable `source_expense_id` on `expenses` (migration `20260626000001` applied to prod — uuid self-FK `ON DELETE SET NULL` + partial index; SQLite v37; models; `upsertWalletEntry` dedupes by `(payer_id, source_expense_id)`). Controller-verified (Decimal-only, 347/347). **Next:** 5b confirm-sheet + share-mirror UI · 5c edit/delete propagation + activity events + wallet badge.
