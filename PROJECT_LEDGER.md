...
- ✅ **DONE (TASK 7, PR #44)** Ghost-row nickname FK violation on `pending_invites` — safe idempotent alignment migration `20260627000001` (drops any FK on the ghost column; re-asserts canonical `add_ghost_member`). Real nickname-conflict path was already hardened by `20260302000001`; **verify ghost invite live after deploy.**
- ✅ **DONE (TASK 7, PR #44)** Google OAuth in-app browser doesn't auto-close — `LaunchMode.platformDefault` (dismissible SFSafariViewController/Custom Tabs) + `closeInAppWebView()` on OAuth callback. **Verify on iOS/Android device.**
