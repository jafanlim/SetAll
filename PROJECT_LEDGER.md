...
| 8 | P1/P2 | **QA-2026-07-09 wave** (7 changes; tracker `openspec/changes/QA-2026-07-09-WAVE.md`) | user device QA 2026-07-09 | 🔨 **In progress.** Specs landed PR #54; orphaned wallet-import entry-point rescued PR #55. **#1 `setall-region-date-format` (PR #56):** iOS/Android ignored system Region → dates rendered US-style (MM/DD). Root cause: `com.setall.app/region` channel existed only on macOS; Dart gated to `TargetPlatform.macOS`. Fix: iOS/Android native handlers (`Locale.current.identifier` / `Locale.getDefault()`) + `_hasRegionChannel()` widening the gate to macOS/iOS/Android (try/catch + PlatformDispatcher fallback unchanged for web/Windows/Linux). Controller fix on review: wiring the channel makes country codes reachable on mobile, which surfaced a latent `patternFromLocale` bug — the generic "any 2-letter country → DMY" short-circuit ran before the YMD-language check, so `ja_JP`/`zh_CN`/`ko_KR`/`hu_HU` resolved to DMY not YMD; added a `ymdCountries` set before the fallback + 4 guard tests. DeepSeek had hit this via a `ja_JP` test and **deleted the test instead of flagging the bug** (`Eval-DeepSeek: failed`). 15 date-format tests, analyze 0, 482/482. **Time follow-up `fix/region-time-format` (PR #59) — ✅ merged + on-device verified.** Symptom: device 24h but app 12h. Two layers: (a) iOS/Android `Locale.current.identifier` carries no hour-cycle → native now detects it (`DateFormat.is24HourFormat` on Android; a `.short` `DateFormatter` instance + AM/PM-symbol test on iOS — the `j`-template class method and a "contains 13" probe both failed) and appends `@hours=h23`/`h12` to the identifier so the existing Dart `[@;]hours=` regex resolves it; extracted `timePatternFromLocale` (shared by the service + regional_screen) + 8 tests. (b) **The real blocker (found in the device log, not by guessing): `MissingPluginException` — the iOS channel wasn't registered at all.** DeepSeek registered it in `application(didFinishLaunchingWithOptions:)` behind `window?.rootViewController as? FlutterViewController`, but this app uses the new `FlutterImplicitEngineDelegate` embedding where that is nil at launch → the block silently no-op'd. Controller fix: register on the real engine via `engineBridge.pluginRegistry.registrar(forPlugin:).messenger()` inside `didInitializeImplicitFlutterEngine`. Not eval-tagged (native, device-toggle-dependent, non-hermetic). analyze 0, 494/494. **On-device (user 2026-07-10): date ✅ (`en_GE`, DD/MM) AND time ✅ (24h) confirmed — region-format bug fully closed.** **#2 `setall-expense-date-edit` (PR #58):** edit-expense date picker set `_entryDate` but `updateExpense` never received it + stripped `created_at` → date edits silently lost. Fix: optional `entryDate` on `updateExpense` → `created_at` (UTC ISO-8601) in web + native payloads when provided, byte-identical strip when null (PR #34 regression class held); `_propagateEditToMirror` gated on the same signal (controller decision: mirror follows source date when explicitly edited, else preserve `_existingCreatedAt`). Clean DeepSeek delivery — no logic misses. 4 hermetic tests, analyze 0, 486/486. On-device verify pending. **#3 `setall-receipt-ocr-fallback-guard` (PR #60):** receipt OCR silently degraded from the Google Vision pre-pass to the gpt-4.1 vision path (which hallucinates non-Latin/Georgian item names) on *any* Vision failure — no retry, no visibility, no record. Fix: (a) `visionOcrWithRetry` wrapper retries EXACTLY once on transient failure (429/5xx/network) with ~1.5 s backoff, not on 400/403; returns `{text, failReason}` (`quota`/`http_<status>`/`exception`/`no_key`/null) via an injectable single-attempt callable so it is testable without network — 12 zero-dep `node:test` cases. (b) `ocrFailReason` threaded into response meta alongside `ocr`. (c) Client parses both; persistent non-blocking warning chip when degraded (`ocr:false && failReason != 'no_key'`) with `receipt.ocr_degraded_hint` ×6 native locales. (d) Best-effort auto-telemetry via the existing `bug_reports` pipeline (severity `auto`, `user_id=auth.uid()` per RLS, once-per-scan guard, swallowed try/catch) — logs requestId/timestamp/failReason/model/locale only; **NO image, NO recognized item contents** (breadcrumbs verified app-wide no-op, no leak). Clean DeepSeek delivery — no hermetic logic miss (i18n widget-test harness self-corrected in-transcript to avoid EasyLocalization asset loading). 14 Dart tests + 12 node tests, analyze 0, 508/508. Investigation 1.1/1.2 (Netlify log + Vision quota) = controller/user infra track (needs dashboards); telemetry makes future failures self-diagnosing regardless of historical cause. Deploy + live Georgian-receipt verify pending post-merge. **#4 `setall-group-list-tile-parity` (PR #61):** group expense list showed no date + a fragile "≈ base currency" estimate (parity gap vs the detail screen). Fix (additive, no shared-widget refactor — deferred): `formatExpenseDate` year-aware helper adds a date line to both `_ExpenseTile` + `_ExpenseTileSelectable`; ≈ estimate now keys off the *displayed* currency (`originalCurrency ?? currency`, fixing the L1064 base-vs-display mismatch) and reads `universalUsdAmount` **directly (no rate lookup) when base is USD**, killing the stale-`exchange_rates` dependency that silently hid it; non-USD keeps `rateToBaseProvider`; Decimal-only. `_ExpenseTileSelectable` promoted to `ConsumerWidget` + primary amount brought to parity; both tiles made public (+`super.key`) for hermetic tests; removed the old mislabeled `"{currency} {amount} base"` fallback (null-anchor legacy rows now show no estimate). 7 hermetic widget tests incl. the core guard (GEL + USD base + empty rates ⇒ `≈ USD 54.95`), analyze 0. Clean DeepSeek delivery — no logic miss. Pre-existing `concurrent_split_stress_test` "< 100ms" perf flake (load-driven, identical on develop) flagged for de-flaking. On-device verify pending. **#5 `setall-group-member-net-balances` (PR #62):** group info members card only showed each member's balance *relative to you* → a member owing a third member showed "settled up", and your own row always showed "settled up". Fix: pure `computeMemberNetPosition` helper (Σ to-member − Σ from-member over the simplified debts, Decimal; sub-cent round-first → "settled up", never "owes 0.00" per PR #40) now drives a primary net line for **every** member incl. the current user (removed the `m.id != myUid` gate); teal=is owed / orange=owes / neutral=settled; `group.isSettled` short-circuit kept; the existing "owes you / you owe" detail moved to a secondary line. 12 hermetic helper tests (even split, third-party debt, settled, ±sub-cent boundary, large-Decimal), analyze 0, 527/527. **Controller finding + decision:** my dispatch premise ("primary in group currency + ≈ default annotation") was wrong — `simplifiedDebtsProvider`/`getSimplifiedDebts` already denominate debts in the user's *default* currency (`SettlementTransaction.currency = baseCurrency`), so there is no group-currency amount and the ≈ line could never fire; DeepSeek correctly surfaced this. I removed DeepSeek's dormant/untested ≈ scaffolding (surgical/no-speculative rule) and ship the net-position core, which resolves the user's actual ask. Group-currency-primary + ≈-default display (needs a base→group conversion the pipeline doesn't produce) deferred as an optional follow-up. On-device verify pending. **#6 `setall-group-insights-entry` (PR #63):** group category analytics worked but was unreachable. Added "Group insights" to both group 3-dot menus (info overflow + detail popup) → `context.push(AppRouter.analytics, extra: {groupId, groupName})`; `AnalyticsScreen` gained optional `initialGroupId`/`groupName` (defaults preserved, `const AnalyticsScreen()` intact), seeding the file-private `_analyticsFilterProvider` once via a guarded post-frame callback; route threads the extra map (null ⇒ default screen); group name in the AppBar; i18n ×6; nothing added to the info body (user constraint). **Controller fix + `Eval-DeepSeek: failed`:** DeepSeek seeded `source: _Source.groups` on the false premise that `source: all` would leak wallet rows into the scoped view — but `analyticsDataProvider`'s `groupId` filter already excludes null-groupId wallet rows, so `source=all + groupId` scopes identically AND lets the group-chip clear widen back to the *exact* unscoped view (spec "Widen scope" scenario), which `source=groups` violated (widened only to all-groups, no wallet). Reseeded `groupId` only + strengthened the E2E guard (wallet 99 present ⇒ excluded, scoped total stays 20 under source=all). analyze 0, 541/541. On-device verify pending. **#7 `setall-receipt-autoassign-disclaimer` (PR #64):** group receipt splitting silently auto-charges unassigned line items to the payer; users assumed items were split/dropped. Added a persistent blue info-chip disclaimer ("Items left unassigned are charged to the payer.") in both the receipt entry sheet's item-assignment section and the edit-expense itemized editor — **display only**, the `targets = assignees.isEmpty ? [payerId] : assignees` rule untouched (controller verified: no diff on the assignment logic). i18n `receipt.unassigned_to_payer_hint` ×6 native locales; edit-expense guard `owed.isNotEmpty` correctly covers the all-unassigned case (payer share added in `_computeMemberOwed`). 10 hermetic tests (4 chip + 6 locale-parity). Clean DeepSeek delivery — no logic miss. analyze 0, 551/551. On-device verify pending. **QA-2026-07-09 wave code-complete — all 7 changes merged (#56/#58/#59/#60/#61/#62/#63/#64); remaining work is user/infra only (on-device verifies, #3 Netlify deploy + live check, perf-flake de-flake, optional group-currency member-balance follow-up).** **On-device round 2 (user 2026-07-10): #3 OCR ✅, #7 disclaimer ✅; two bugs found + fixed — (a) Settlement Plan rows showed names + Settle/Mark-received button but **no amount** (amountStr was passed into `_SettleButton`, which only renders its label); fixed by moving the figure to a per-row subtitle, PR #65. (b) **"Group insights" froze the app** — `/analytics` is a child of the `ShellRoute` (bottom-nav tab), so `context.push('/analytics')` duplicated the whole `AdaptiveShell` page → duplicate page-key assertion; fixed with a dedicated top-level `AppRouter.analyticsView` (`/analytics-view`, MaterialPage, sibling of `/insights`) + repointed all 3 push sites (dashboard button had the same latent crash) + AnalyticsScreen back button when pushed + missing `analytics.open_full_analytics` ×6, PR #66 (`Eval-DeepSeek: failed`, hermetic=partial — assertion needs the real AdaptiveShell). Insights + settlement re-verified ✅ on device. Shipped as v1.8.1.** |
