...
| 9 | P1 | `setall-region-date-format` | user-reported 2026-07-09 | 🔨 PR #56 open — iOS/Android ignored system Region, dates rendered US-style (MM/DD). Root cause: `com.setall.app/region` MethodChannel only existed on macOS; Dart gated to `TargetPlatform.macOS`. Fix: added iOS/Android native handlers (`Locale.current.identifier` / `Locale.getDefault()`) + widened Dart gate to macOS/iOS/Android (try/catch + PlatformDispatcher fallback unchanged for web/Windows/Linux). 10 new unit tests for `_patternFromLocale` + `jm()` skeleton fallback. On-device verify pending (iPhone en + Region Georgia). |
