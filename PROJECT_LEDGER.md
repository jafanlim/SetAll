...
| 5 | P1 | `setall-shared-expense-wallet-share` | §4 item 2 | 🔨 **5a data-layer DONE** (PR #32) — `source_expense_id` link + dedupe. Migration `20260626000001` **applied to prod** (uuid self-FK `ON DELETE SET NULL` + partial index `(payer_id, source_expense_id)`), SQLite v37, models, `upsertWalletEntry` dedupes by `(payer_id, source_expense_id)`. DeepSeek logic clean, controller-verified (Decimal-only, 347/347). **Next:** 5b confirm-sheet + share-mirror UI · 5c edit/delete propagation + activity + wallet badge |
